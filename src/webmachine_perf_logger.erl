...
format_req(#wm_log_data{req_id=ReqId,
    ...
    ValidEndTime = if EndTime =:= undefined -> os:timestamp(); true -> EndTime end,
    TTPD = webmachine_util:now_diff_milliseconds(ValidEndTime, StartTime),
    TTPS = webmachine_util:now_diff_milliseconds(FinishTime, ValidEndTime),
